...
def color_thresh(img, rgb_thresh=(170, 170, 170), rgb_thresh_max=(255, 255, 255)):
...
def perception_step(Rover):
    ...
    # Define calibration box in source (actual) and destination (desired) coordindates
    ...
    ycenter = image.shape[0] - bottom_offset
    source = np.float32([[14, 140], [301, 140], [200, 96], [118, 96]])
    destination = np.float32([[xcenter - dst_size, ycenter],
                              [xcenter + dst_size, ycenter],
                              [xcenter + dst_size, ycenter - 2*dst_size],
                              [xcenter - dst_size, ycenter - 2*dst_size],
                              ])
    ...
    #check if we should update worlmap, if our roll or pitch is > 0.5
    #don't update
    doUpdateWorldMap = (Rover.roll < 0.5 or Rover.roll > 359.5) and \
                       (Rover.pitch < 0.5 or Rover.pitch > 359.5)
    if doUpdateWorldMap:
        Rover.worldmap[y_world, x_world, 2] += 50
        Rover.worldmap[obs_y_world, obs_x_world, 0] += 50
